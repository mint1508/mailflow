...
import { getCpanelConfig, resetCpanelMailboxPassword } from './cpanelClient.js';
...
export async function createMailboxActivation({ actorUserId, mailbox, contactEmail }) {
  ...
  try {
    ...
    const accountResult = await client.query(
      ...
      [actorUserId, mailboxEmail, config.host, encrypt(mailbox.password)],
    );
...
export async function createExistingMailboxActivation({ actorUserId, mailboxEmail, contactEmail }) {
  ...
  const existingUser = await query('SELECT id FROM users WHERE lower(username) = $1', [normalized]);
  if (existingUser.rows.length) throw new Error('A MailFlow user already exists for this mailbox');

  let temporaryPassword = null;
  const existingAccount = await query(
    'SELECT id FROM email_accounts WHERE lower(email_address) = $1 ORDER BY created_at LIMIT 1',
    [normalized],
  );
  if (!existingAccount.rows.length) {
    temporaryPassword = (await resetCpanelMailboxPassword(normalized)).password;
  }
  ...
  try {
    ...
    if (existingAccount.rows.length) {
      ...
      await client.query(
        'UPDATE email_accounts SET managed_mailbox = true, enabled = false WHERE id = $1',
        [accountId],
      );
    } else {
      const inserted = await client.query(
        `INSERT INTO email_accounts (
         ...
         ) VALUES ($1,$2,$2,'#6366f1','imap',$3,993,true,$3,465,'SSL',$2,$4,$2,false,true)
         RETURNING id`,
        [actorUserId, normalized, config.host, encrypt(temporaryPassword)],
      );
